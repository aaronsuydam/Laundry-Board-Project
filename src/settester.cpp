...
#include <set>
#include <ctime>
#include "C:\Users\andre\repos2\Sessions.cpp"
using namespace std;
...
class SetImplementation
{
    ...
    vector<SetUser> generateSetVector()
    {
        ...
        //set<int, SetUser>::iterator iter;
        // vector<SetUser*> setVector(mset.begin(), mset.end());

         std::vector<SetUser> setVector(mset.size());
         std:copy(mset.begin(), mset.end(), setVector.begin());
    ...
    double totalAverageWashing(SetUser* root)
    {
    ...
    double totalAverageDrying(SetUser* root)
    {
        vector<SetUser> setUsers = generateSetVector();
